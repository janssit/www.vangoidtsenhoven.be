...
var aLoadImgSrc = '/components/com_picman/libraries/shadowbox/aBox-loading.gif'; // where is the loading image? (recommend absolute)
var siteURL = 'http://www.vangoidtsenhoven.be/'; // requires trailing / (after .com)
...
YAHOO.awesomebox = function() {
	return {
		...
		init : function() {
			...
			$(aImgLink).setAttribute('id', 'aImgLink');
			$(aImgLink).setAttribute('href', '#');
			aImgHolder.appendChild(aImgLink);

			var aImg = document.createElement('img');
			$(aImg).setAttribute('id', 'aImg');
			aImgLink.appendChild(aImg);
			$D.setStyle('aImg', 'opacity', '0');
			$D.setStyle('aImg', 'visibility', 'hidden');
		...
		load : function(e, newImg) {
			...
			aPreload.onload = function() {
				...
				if(
					that.getElementsByTagName('img')[0] &&
					that.getElementsByTagName('img')[0].getAttribute('longdesc') != null
				) {
					var aLink = that.getElementsByTagName('img')[0].getAttribute('longdesc');
				}
				if($D.hasClass('aBoxMeta', 'aOpen')) {
					setTimeout(function() {
						$('aInfoTitle').innerHTML = aInfoTitle;
						$('aInfo').innerHTML = aInfo;
						$('aImgLink').setAttribute('href', aLink);
					}, 250);
				} else {
					$('aInfoTitle').innerHTML = aInfoTitle;
					$('aInfo').innerHTML = aInfo;
